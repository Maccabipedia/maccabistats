...
def plotly_horizontal_bar_chart(best_players: list[Tuple]):
    ...
    fig = go.Figure(
        go.Bar(x=names, y=sizes, orientation="v", text=sizes, textposition="outside", marker=dict(color=colors))
    )
...
if __name__ == "__main__":
    ...
    most_played_with_others_player = sorted(
        players_to_available_players_len.items(), key=lambda item: item[1], reverse=True
    )
