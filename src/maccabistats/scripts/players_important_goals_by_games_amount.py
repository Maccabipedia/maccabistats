...
class PlayersImportantGoals:
    ...
    def players_important_goals_per_game_and_games(self) -> dict[str, Tuple[int, int]]:
        return {
            self._remove_per_game_text(player_name): (
                ...
                self.player_to_played_games.get(self._remove_per_game_text(player_name), 1),
            )
            for player_name, player_goals_per_game in self.player_to_important_goals_per_game.items()
        }
    ...
    def players_important_goals_last_minutes_per_game_and_games(self) -> dict[str, Tuple[int, int]]:
        return {
            ...
            for player_name, player_goals_per_game in self.player_to_important_goals_last_minutes_per_game.items()
        }
    ...
    def players_important_goals_percentage_from_games_and_games(self) -> dict[str, Tuple[int, int]]:
        return {
            ...
            for player_name, player_goals_per_game in self.player_to_important_goals_percentage_from_total_goals.items()
        }
...
def get_players_important_goals_stats() -> PlayersImportantGoals:
    ...
    return PlayersImportantGoals(
        ...
        player_to_important_goals_last_minutes_per_game=dict(
            maccabi_games.important_goals.get_top_players_for_goals_in_last_minutes_per_game(0, 1, minimum_games=20)
        ),
...
def show_player_important_goals_last_minutes_per_game_to_games_graph(players_stats: PlayersImportantGoals):
    ...
    show_graph_from_player_stat_to_games(
        players_stats.players_important_goals_last_minutes_per_game_and_games,
        names_to_positions={
            "נתן פנץ": (13, -7),
            "ציון צמח": (0, 10),
            "יעקב נומדר": (-20, -13),
...
def show_player_important_goals_percentage_from_goals_to_games_graph(players_stats: PlayersImportantGoals):
    ...
    show_graph_from_player_stat_to_games(
        players_stats.players_important_goals_percentage_from_games_and_games,
        names_to_positions={
...
def show_graph_from_player_stat_to_games(
    ...
    for player_name, player_values in list(player_stats_to_games.items())[:_PLAYERS_TO_SHOW_NAME]:
        ...
        plt.annotate(bidialg.get_display(player_name), xy=player_values, xytext=position, textcoords="offset points")
