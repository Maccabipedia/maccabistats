...
def show_players_with_atleast_goals_and_assists_in_this_season(
    maccabi_season, atleast_goals_and_assist=10, maximum_players_to_show=3
):
    """
    ...
    for player_name in players_top_scorers_and_assisters:
        players_with_minimum_between_scorers_and_assisters[player_name] = min(
            scorers[player_name], assisters[player_name]
        )

    maybe_top_players = sorted(
        players_with_minimum_between_scorers_and_assisters.items(), key=lambda item: item[1], reverse=True
    )[:maximum_players_to_show]
    top_players_names = [
        player_info[0] for player_info in maybe_top_players if player_info[1] >= atleast_goals_and_assist
    ]
...
def show_players_that_done_a_double_double():
    ...
    for season_index in reversed(range(len(maccabipedia_seasons))):
        show_players_with_atleast_goals_and_assists_in_this_season(
            maccabipedia_seasons[season_index], atleast_goals_and_assist=10
        )
