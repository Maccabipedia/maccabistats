...
class MaccabiGamesSeasonsStats:
    ...
    def __init__(self, maccabi_games_stats: MaccabiGamesStats):
        ...
        if len(self.maccabi_games_stats.available_seasons) == 1:
            ...
            self._seasons_dict = {
                season: self.maccabi_games_stats.get_games_by_season(season)
                for season in self.maccabi_games_stats.available_seasons
            }
    ...
    def __repr__(self) -> str:
        ...
        ordered_seasons = pprint.pformat(
            [
                f"{season: <7} ({self._current_sort_attribute_function(self[season])})"
    ...
    def __getitem__(self, item: int | str) -> MaccabiGamesStats:
        """
    ...
    def _refresh_sorting(
        self, sort_attribute_function: Callable[[MaccabiGamesStats], Any], sort_attribute_description: str
    ) -> None:
        """
        ...
        self._seasons_dict = OrderedDict(
            sorted(
                self._seasons_dict.items(),
                key=lambda item: self._current_sort_attribute_function(item[1]),
                reverse=True,
            )
        )
    ...
    def sort_by_games_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s), sort_attribute_description="sort by games count"
        )

    def sort_by_wins_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.wins_count, sort_attribute_description="sort by wins count"
        )

    def sort_by_wins_percentage(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.wins_percentage,
            sort_attribute_description="sort by wins percentage",
        )

    def sort_by_losses_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.losses_count, sort_attribute_description="sort by losses count"
        )

    def sort_by_losses_percentage(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.losses_percentage,
            sort_attribute_description="sort by losses percentage",
        )

    def sort_by_ties_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.ties_count, sort_attribute_description="sort by ties count"
        )

    def sort_by_ties_percentage(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.ties_percentage,
    ...
    def sort_by_goals_ratio(self) -> None:
        ...
        """
        self._refresh_sorting(
            sort_attribute_function=lambda s: s.results.goals_ratio,
    ...
    def sort_by_captains_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s.players.most_captains),
    ...
    def sort_by_assisted_players_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s.players.best_assisters),
    ...
    def sort_by_goal_involved_players_count(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s.players.most_goals_involved),
    ...
    def sort_by_penalties_scorers_amount(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s.players.best_scorers_by_penalty),
            sort_attribute_description="sort by the number of different penalties scorers",
        )

    def sort_by_comebacks_to_win_amount(self) -> None:
        self._refresh_sorting(
            sort_attribute_function=lambda s: len(s.comebacks.won_from_any_goal_diff()),
            sort_attribute_description="sort by the number of comebacks to winning",
        )
    ...
    def sort_by_potential_comebacks_that_maccabi_didnt_win(self) -> None:
        self._refresh_sorting(
            ...
            sort_attribute_description="sort by the number of potential comebacks that maccabi didn't win",
        )
