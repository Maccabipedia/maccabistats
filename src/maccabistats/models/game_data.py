...
class GameData(object):
    ...
    def played_before(self, date: datetime.datetime | datetime.date | str) -> bool:
        if isinstance(date, str):
            date = datetime_parser(date)

        return date >= self.date

    def played_after(self, date: datetime.datetime | datetime.date | str) -> bool:
        if isinstance(date, str):
            date = datetime_parser(date)
    ...
    def __get_date_as_datetime(self) -> datetime.datetime:
        ...
        return datetime.datetime(
            year=int(date_args[2]), month=GameData.__get_month_num_from_hebrew(date_args[1]), day=int(date_args[0])
        )

    @staticmethod
    def __get_month_num_from_hebrew(month_name: str) -> int:
        months_in_hebrew_to_num = {
            "ינו": 1,
            "פבר": 2,
            "מרץ": 3,
            "אפר": 4,
    ...
    @property
    def league_fixture(self) -> int | None:
        try:
    ...
    def events(self) -> list[dict[str, Any]]:
        ...
        players_events = [
            dict(
                player.get_as_normal_player().__dict__,  # Players attributes, normal -> no events.
                **event.json_dict(),
