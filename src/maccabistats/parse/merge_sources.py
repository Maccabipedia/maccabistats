...
def merge_maccabitlv_and_table(maccabitlv_site_source, table_source):
    ...
    for maccabitlv_site_game in maccabitlv_site_league_games:
        ...
        if len(matching_games) != 1:
            logger.warning(
                "Could not decide which game to take from this date: {date}, Found {num} games, adding them anyway".format(
                    date=maccabitlv_site_game.date, num=len(matching_games)
                )
            )
        ...
        merged_games.append(
            __merge_two_games(matching_games[0], maccabitlv_site_game)
        )  # Takes only the first game on this date.
...
def __merge_two_games(table_game, maccabitlv_site_game):
    ...
    if (
        table_game.away_team.name == maccabitlv_site_game.home_team.name
        and table_game.home_team.name == maccabitlv_site_game.away_team.name
    ):
        logger.info(
            f"Found home-away mismatch - switching positions,"
            f" new home team: {maccabitlv_site_game.away_team.name}, new away team: {maccabitlv_site_game.home_team.name}"
        )
        maccabitlv_site_game.home_team, maccabitlv_site_game.away_team = (
            maccabitlv_site_game.away_team,
            maccabitlv_site_game.home_team,
        )

    if table_game.away_team.score != maccabitlv_site_game.away_team.score:
        logger.info(
            f"Found diff - away team score: {table_game.away_team.score}(table) - {maccabitlv_site_game.away_team.score}(maccabitlv site)"
        )
    ...
    if table_game.home_team.score != maccabitlv_site_game.home_team.score:
        logger.info(
            f"Found diff - home team score: {table_game.home_team.score}(table) - {maccabitlv_site_game.home_team.score}(maccabitlv site)"
        )
...
def __override_general_game_details_from_table(maccabitlv_site_game, table_game):
    ...
    if table_game.referee != maccabitlv_site_game.referee:
        logger.info(
            f"Found diff - referee: {table_game.referee}(table) - {maccabitlv_site_game.referee}(maccabitlv site)"
        )
        maccabitlv_site_game.referee = table_game.referee

    if table_game.stadium != maccabitlv_site_game.stadium:
        logger.info(
            f"Found diff - stadium: {table_game.stadium}(table) - {maccabitlv_site_game.stadium}(maccabitlv site)"
        )
        maccabitlv_site_game.stadium = table_game.stadium

    if table_game.fixture != maccabitlv_site_game.fixture:
        logger.info(
            f"Found dif - fixture: {table_game.fixture}(table) - {maccabitlv_site_game.fixture}(maccabitlv site)"
        )
