...
_referees_name_fixes = [
    ("איתן שמואלביץ", ["איתן שמואלביץ'", "שמואלביץ איתן"]),
...
_competitions_name_fixes = [
    ("גביע אירופה למחזיקות גביע", ["גביע אירופה למחזיקות"]),
    ...
    (
        "ליגת העל",
        ["ליגת Winner", "ליגת הבורסה לניירות ערך", "ליגת ג׳פניקה", "ליגת one zero הבנק הדיגיטלי", "ליגת WINNER"],
    ),
...
def __fix_teams_names(game):
    if game.not_maccabi_team.name in teams_names_changer:
        ...
        if old_team_name != game.not_maccabi_team.name:
            logger.info(
                f"Changing {'Home' if game.is_maccabi_home_team else 'Away'} team name from: "
