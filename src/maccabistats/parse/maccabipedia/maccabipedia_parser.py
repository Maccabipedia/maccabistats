...
MACCABI_PEDIA_EVENTS = defaultdict(
    _unknown_event,
    {
        1: defaultdict(
            _unknown_event, {_EMPTY_SUB_EVENT: GameEventTypes.LINE_UP, 111: GameEventTypes.LINE_UP}
        ),  # Special for GK
        2: defaultdict(_unknown_event, {_EMPTY_SUB_EVENT: GameEventTypes.BENCHED, 211: GameEventTypes.BENCHED}),
        ...
        8: defaultdict(
            _unknown_event,
            {
                81: _DUPLICATE_MACCABIPEDIA_EVENT,
...
MACCABIPEDIA_GOALS_TYPE = {
    30: GoalTypes.UNCATEGORIZED,
...
MACCABIPEDIA_ASSISTS_TYPE = {
    40: AssistTypes.UNCATEGORIZED,
...
class MaccabiPediaParser(object):
    def __init__(self):
        ...
        [
            self._games_events_by_game[game_event[_PAGE_NAME_FIELD_NAME]].append(game_event)
            for game_event in self._games_events_as_json
        ]

    @staticmethod
    def _get_games_metadata():
        return [
            game_metadata_as_json for game_metadata_as_json in MaccabiPediaCargoChunksCrawler.create_games_crawler()
        ]

    @staticmethod
    def _get_games_events():
        return [
            game_events_as_json for game_events_as_json in MaccabiPediaCargoChunksCrawler.create_games_events_crawler()
        ]
    ...
    def _parse_player_event(self, player_event):
        ...
        if GameEventTypes.GOAL_ASSIST == MACCABI_PEDIA_EVENTS[player_event["EventType"]]:
            return AssistGameEvent(
                time_occur=event_time, assist_type=MACCABIPEDIA_ASSISTS_TYPE[player_event["SubType"]]
            )
    ...
    def _extract_players_events_for_team(self, game_events_as_json, log_errors: bool):
        ...
        [
            players_events_by_name[player_event["PlayerName"]].append(player_event)
            for player_event in game_events_as_json
        ]
        ...
        for player_name, player_json_events in players_events_by_name.items():
            ...
            if log_errors and len(player_number) > 1:
                ...
                if len(non_empty_number) > 1:
                    logger.warning(
                        f"Found more than 1 player_number for player: {player_name}, "
                    ...
                    logger.warning(
                        f"Player: {player_name} has a number in this game: {non_empty_number}, "
    ...
    def _build_maccabistats_game(self, game_metadata, game_events):
        ...
        maccabi_players = self._extract_players_events_for_team(
            [event for event in game_events if event["Team"] == _MACCABI_TEAM], log_errors=True
        )
        maccabi_team = TeamInGame(
            "מכבי תל אביב", game_metadata["CoachMaccabi"], game_metadata["ResultMaccabi"], maccabi_players
        )

        not_maccabi_players = self._extract_players_events_for_team(
            [event for event in game_events if event["Team"] == _NOT_MACCABI_TEAM], log_errors=False
        )

        not_maccabi_team = TeamInGame(
            game_metadata["Opponent"],
            game_metadata["CoachOpponent"],
            game_metadata["ResultOpponent"],
            not_maccabi_players,
        )

        home_team, away_team = (
            (maccabi_team, not_maccabi_team) if game_metadata["HomeAway"] == "בית" else (not_maccabi_team, maccabi_team)
        )
        ...
        return GameData(
            competition=game_metadata["Competition"],
            fixture=game_metadata["Leg"],
            date_as_hebrew_string="",
            stadium=game_metadata["Stadium"],
            crowd=game_metadata["Crowd"],
            referee=game_metadata["Refs"],
            home_team=home_team,
            away_team=away_team,
            season_string=str(game_metadata["Season"]),
            half_parsed_events=[],
    ...
    def parse(self):
        ...
        for game_name in self._game_metadata_by_game.keys():
            ...
            parsed_games.append(
                self._build_maccabistats_game(
                    self._game_metadata_by_game[game_name][0], self._games_events_by_game[game_name]
                )
            )
