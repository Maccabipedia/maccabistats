...
class MaccabiSiteTeamParser(object):
    ...
    def __parse_player(player_bs_content, is_line_up):
        ...
        for goal_minute in player_bs_content.select_one("div.goals").get_text().split():
            events.append(GoalGameEvent(MaccabiSiteTeamParser.__strip_geresh_as_timedelta(goal_minute)))
        ...
        for card_event_time, card_img_bs in cards:
            ...
            if card_link.endswith("yellow.png"):
                player_events.append(
                    GameEvent(
                        GameEventTypes.YELLOW_CARD, MaccabiSiteTeamParser.__strip_geresh_as_timedelta(card_event_time)
                    )
                )
            elif card_link.endswith("red.png"):
                player_events.append(
                    GameEvent(
                        GameEventTypes.RED_CARD, MaccabiSiteTeamParser.__strip_geresh_as_timedelta(card_event_time)
                    )
                )
            else:
    ...
    def __append_substitution_events_for_player(player_bs_content, player_events, is_line_up):
        ...
        if is_line_up:
            player_events.append(
                GameEvent(
                    GameEventTypes.SUBSTITUTION_OUT,
