...
def __get_player_for_half_parsed_goals_events(game, goal_event):
    r"""
    Trying to search for exact name, after that for first or last name, after that splitting name by dot (if exist) and search for first \ last name.
    """
    ...
    logger.info("Cant find full player name, trying first or last name:{name}".format(name=name))
    ...
    logger.info("Cant find first or last player name, trying check for dot pattern name:{name}".format(name=name))
    if "." in name:
